...
export const getSubdomainsByDomainTool = defineTool<BackstageMcpContext>()({
  name: BackstageToolName.GET_SUBDOMAINS_BY_DOMAIN,
  title: 'Get subdomains by domain',
  description: 'Get direct child Domains; recursive true includes nested subdomains.',
  ...
  async handler({ input, context }) {
    return catalogResult(
      collectDescendants(context.catalogClient.queryEntities.bind(context.catalogClient), {
        ...
        kind: BackstageEntityKind.DOMAIN,
        recursive: input.recursive,
      }),
      'Backstage could not complete the get subdomains by domain lookup'
    );
  },
});
