...
const INPUT = { providerRef: 'component:default/smoke-service' };

describe('get_apis_by_provider tool', () => {
  it('should expose a valid read-only contract and send a filtered Catalog request', async () => {
    expect(getApisByProviderTool.annotations).toMatchObject({ readOnlyHint: true, destructiveHint: false });
    expect(getApisByProviderTool.inputSchema.safeParse(INPUT).success).toBe(true);
    expect(getApisByProviderTool.inputSchema.safeParse({ unexpected: true }).success).toBe(false);
    const fetcher = vi
    ...
    try {
      const result = await connection.client.callTool({ name: 'get_apis_by_provider', arguments: INPUT });
      ...
      if (request instanceof Request) {
        ...
        expect(decodeURIComponent(request.url)).toContain('relations.apiProvidedBy=component:default/smoke-service');
  ...
  it('should map Catalog rejection to a typed MCP error', async () => {
    ...
    const connection = await connectTestClient(createBackstageServer({ catalogClient, logger: noopLogger }));
    try {
      expect(await connection.client.callTool({ name: 'get_apis_by_provider', arguments: INPUT })).toMatchObject({
        isError: true,
