...
import { providerLookupInputSchema, successOutputSchema } from '../../shared/schema.js';
...
export const getApisByProviderTool = defineTool<BackstageMcpContext>()({
  name: BackstageToolName.GET_APIS_BY_PROVIDER,
  title: 'Get APIs by provider',
  description: 'Get APIs provided by the specified entity through apiProvidedBy.',
  inputSchema: providerLookupInputSchema,
  ...
  async handler({ input, context }) {
    return catalogResult(
      queryRelatedEntities(context.catalogClient.queryEntities.bind(context.catalogClient), {
        relation: BackstageRelation.API_PROVIDED_BY,
        targetRef: input.providerRef,
        kind: BackstageEntityKind.API,
      }),
      'Backstage could not complete the get apis by provider lookup'
    );
  },
});
