...
describe('get_entities_by_domain tool', () => {
  it('should expose a valid read-only contract and send a filtered Catalog request', async () => {
    expect(getEntitiesByDomainTool.annotations).toMatchObject({ readOnlyHint: true, destructiveHint: false });
    expect(getEntitiesByDomainTool.inputSchema.safeParse(INPUT).success).toBe(true);
    expect(getEntitiesByDomainTool.inputSchema.safeParse({ unexpected: true }).success).toBe(false);
    const fetcher = vi
    ...
    try {
      const result = await connection.client.callTool({ name: 'get_entities_by_domain', arguments: INPUT });
  ...
  it('should map Catalog rejection to a typed MCP error', async () => {
    ...
    const connection = await connectTestClient(createBackstageServer({ catalogClient, logger: noopLogger }));
    try {
      expect(await connection.client.callTool({ name: 'get_entities_by_domain', arguments: INPUT })).toMatchObject({
        isError: true,
