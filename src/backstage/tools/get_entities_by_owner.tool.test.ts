...
const INPUT = { ownerRef: 'group:default/smoke' };

describe('get_entities_by_owner tool', () => {
  it('should expose a valid read-only contract and send a filtered Catalog request', async () => {
    expect(getEntitiesByOwnerTool.annotations).toMatchObject({ readOnlyHint: true, destructiveHint: false });
    expect(getEntitiesByOwnerTool.inputSchema.safeParse(INPUT).success).toBe(true);
    expect(getEntitiesByOwnerTool.inputSchema.safeParse({ unexpected: true }).success).toBe(false);
    const fetcher = vi
    ...
    try {
      const result = await connection.client.callTool({ name: 'get_entities_by_owner', arguments: INPUT });
      ...
      if (request instanceof Request) {
        ...
        expect(decodeURIComponent(request.url)).toContain('relations.ownedBy=group:default/smoke');
  ...
  it('should map Catalog rejection to a typed MCP error', async () => {
    ...
    const connection = await connectTestClient(createBackstageServer({ catalogClient, logger: noopLogger }));
    try {
      expect(await connection.client.callTool({ name: 'get_entities_by_owner', arguments: INPUT })).toMatchObject({
        isError: true,
