...
/** Get dependents by entity. */
export const getDependentsByEntityTool = defineTool<BackstageMcpContext>()({
  name: BackstageToolName.GET_DEPENDENTS_BY_ENTITY,
  title: 'Get dependents by entity',
  description: 'Get entities that explicitly depend on the target through dependsOn.',
  ...
  async handler({ input, context }) {
    return catalogResult(
      queryRelatedEntities(context.catalogClient.queryEntities.bind(context.catalogClient), {
        relation: BackstageRelation.DEPENDS_ON,
        targetRef: input.entityRef,
      }),
      'Backstage could not complete the get dependents by entity lookup'
    );
  },
});
