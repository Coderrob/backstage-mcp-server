...
import { domainInventoryInputSchema, successOutputSchema } from '../../shared/schema.js';
...
export const getEntitiesByDomainTool = defineTool<BackstageMcpContext>()({
  name: BackstageToolName.GET_ENTITIES_BY_DOMAIN,
  title: 'Get entities by domain',
  description:
    'Get all visible descendants of a Domain through partOf by default; recursive false returns direct children.',
  inputSchema: domainInventoryInputSchema,
  ...
  async handler({ input, context }) {
    return catalogResult(
      collectDescendants(context.catalogClient.queryEntities.bind(context.catalogClient), {
        ...
        targetRef: input.domainRef,
        recursive: input.recursive,
      }),
      'Backstage could not complete the get entities by domain lookup'
    );
  },
});
