...
import { domainLookupInputSchema, successOutputSchema } from '../../shared/schema.js';
...
export const getSystemsByDomainTool = defineTool<BackstageMcpContext>()({
  name: BackstageToolName.GET_SYSTEMS_BY_DOMAIN,
  title: 'Get systems by domain',
  description: 'Get direct Systems in a Domain; recursive true includes Systems within nested subdomains.',
  ...
  async handler({ input, context }) {
    return catalogResult(
      collectDescendants(context.catalogClient.queryEntities.bind(context.catalogClient), {
        ...
        kind: BackstageEntityKind.SYSTEM,
        recursive: input.recursive,
      }),
      'Backstage could not complete the get systems by domain lookup'
    );
  },
});
