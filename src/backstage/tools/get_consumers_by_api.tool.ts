...
import { apiLookupInputSchema, successOutputSchema } from '../../shared/schema.js';
...
/** Get consumers by API. */
export const getConsumersByApiTool = defineTool<BackstageMcpContext>()({
  name: BackstageToolName.GET_CONSUMERS_BY_API,
  title: 'Get consumers by API',
  description: 'Get entities that explicitly consume the API through consumesApi.',
  inputSchema: apiLookupInputSchema,
  ...
  async handler({ input, context }) {
    return catalogResult(
      queryRelatedEntities(context.catalogClient.queryEntities.bind(context.catalogClient), {
        relation: BackstageRelation.CONSUMES_API,
        targetRef: input.apiRef,
      }),
      'Backstage could not complete the get consumers by api lookup'
    );
  },
});
