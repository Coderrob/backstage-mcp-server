...
const INPUT = { entityRef: 'component:default/smoke-service' };

describe('get_dependencies_by_entity tool', () => {
  it('should expose a valid read-only contract and send a filtered Catalog request', async () => {
    expect(getDependenciesByEntityTool.annotations).toMatchObject({ readOnlyHint: true, destructiveHint: false });
    expect(getDependenciesByEntityTool.inputSchema.safeParse(INPUT).success).toBe(true);
    expect(getDependenciesByEntityTool.inputSchema.safeParse({ unexpected: true }).success).toBe(false);
    const fetcher = vi
    ...
    try {
      const result = await connection.client.callTool({ name: 'get_dependencies_by_entity', arguments: INPUT });
      ...
      if (request instanceof Request) {
        ...
        expect(decodeURIComponent(request.url)).toContain('relations.dependencyOf=component:default/smoke-service');
  ...
  it('should map Catalog rejection to a typed MCP error', async () => {
    ...
    const connection = await connectTestClient(createBackstageServer({ catalogClient, logger: noopLogger }));
    try {
      expect(await connection.client.callTool({ name: 'get_dependencies_by_entity', arguments: INPUT })).toMatchObject({
        isError: true,
