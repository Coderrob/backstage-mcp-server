...
describe('get_owners_by_entity tool', () => {
  it('should expose a valid read-only contract and send a filtered Catalog request', async () => {
    expect(getOwnersByEntityTool.annotations).toMatchObject({ readOnlyHint: true, destructiveHint: false });
    expect(getOwnersByEntityTool.inputSchema.safeParse(INPUT).success).toBe(true);
    expect(getOwnersByEntityTool.inputSchema.safeParse({ unexpected: true }).success).toBe(false);
    const fetcher = vi
    ...
    try {
      const result = await connection.client.callTool({ name: 'get_owners_by_entity', arguments: INPUT });
      ...
      if (request instanceof Request) {
        ...
        expect(decodeURIComponent(request.url)).toContain('relations.ownerOf=component:default/smoke-service');
  ...
  it('should map Catalog rejection to a typed MCP error', async () => {
    ...
    const connection = await connectTestClient(createBackstageServer({ catalogClient, logger: noopLogger }));
    try {
      expect(await connection.client.callTool({ name: 'get_owners_by_entity', arguments: INPUT })).toMatchObject({
        isError: true,
