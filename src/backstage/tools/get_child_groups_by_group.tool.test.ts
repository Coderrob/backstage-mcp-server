...
const INPUT = { groupRef: 'group:default/smoke' };

describe('get_child_groups_by_group tool', () => {
  it('should expose a valid read-only contract and send a filtered Catalog request', async () => {
    expect(getChildGroupsByGroupTool.annotations).toMatchObject({ readOnlyHint: true, destructiveHint: false });
    expect(getChildGroupsByGroupTool.inputSchema.safeParse(INPUT).success).toBe(true);
    expect(getChildGroupsByGroupTool.inputSchema.safeParse({ unexpected: true }).success).toBe(false);
    const fetcher = vi
    ...
    try {
      const result = await connection.client.callTool({ name: 'get_child_groups_by_group', arguments: INPUT });
      ...
      if (request instanceof Request) {
        ...
        expect(decodeURIComponent(request.url)).toContain('relations.childOf=group:default/smoke');
  ...
  it('should map Catalog rejection to a typed MCP error', async () => {
    ...
    const connection = await connectTestClient(createBackstageServer({ catalogClient, logger: noopLogger }));
    try {
      expect(await connection.client.callTool({ name: 'get_child_groups_by_group', arguments: INPUT })).toMatchObject({
        isError: true,
