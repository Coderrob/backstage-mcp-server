...
describe('get_users_by_group tool', () => {
  it('should expose a valid read-only contract and send a filtered Catalog request', async () => {
    expect(getUsersByGroupTool.annotations).toMatchObject({ readOnlyHint: true, destructiveHint: false });
    expect(getUsersByGroupTool.inputSchema.safeParse(INPUT).success).toBe(true);
    expect(getUsersByGroupTool.inputSchema.safeParse({ unexpected: true }).success).toBe(false);
    const fetcher = vi
    ...
    try {
      const result = await connection.client.callTool({ name: 'get_users_by_group', arguments: INPUT });
      ...
      if (request instanceof Request) {
        ...
        expect(decodeURIComponent(request.url)).toContain('relations.memberOf=group:default/smoke');
  ...
  it('should map Catalog rejection to a typed MCP error', async () => {
    ...
    const connection = await connectTestClient(createBackstageServer({ catalogClient, logger: noopLogger }));
    try {
      expect(await connection.client.callTool({ name: 'get_users_by_group', arguments: INPUT })).toMatchObject({
        isError: true,
