...
const INPUT = { userRef: 'user:default/smoke' };

describe('get_groups_by_user tool', () => {
  it('should expose a valid read-only contract and send a filtered Catalog request', async () => {
    expect(getGroupsByUserTool.annotations).toMatchObject({ readOnlyHint: true, destructiveHint: false });
    expect(getGroupsByUserTool.inputSchema.safeParse(INPUT).success).toBe(true);
    expect(getGroupsByUserTool.inputSchema.safeParse({ unexpected: true }).success).toBe(false);
    const fetcher = vi
    ...
    try {
      const result = await connection.client.callTool({ name: 'get_groups_by_user', arguments: INPUT });
      ...
      if (request instanceof Request) {
        ...
        expect(decodeURIComponent(request.url)).toContain('relations.hasMember=user:default/smoke');
  ...
  it('should map Catalog rejection to a typed MCP error', async () => {
    ...
    const connection = await connectTestClient(createBackstageServer({ catalogClient, logger: noopLogger }));
    try {
      expect(await connection.client.callTool({ name: 'get_groups_by_user', arguments: INPUT })).toMatchObject({
        isError: true,
