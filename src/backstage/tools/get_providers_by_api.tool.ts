...
/** Get providers by API. */
export const getProvidersByApiTool = defineTool<BackstageMcpContext>()({
  name: BackstageToolName.GET_PROVIDERS_BY_API,
  title: 'Get providers by API',
  description: 'Get entities that explicitly provide the API through providesApi.',
  ...
  async handler({ input, context }) {
    return catalogResult(
      queryRelatedEntities(context.catalogClient.queryEntities.bind(context.catalogClient), {
        relation: BackstageRelation.PROVIDES_API,
        targetRef: input.apiRef,
      }),
      'Backstage could not complete the get providers by api lookup'
    );
  },
});
