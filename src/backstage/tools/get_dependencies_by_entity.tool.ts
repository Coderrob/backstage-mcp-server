...
/** Get dependencies by entity. */
export const getDependenciesByEntityTool = defineTool<BackstageMcpContext>()({
  name: BackstageToolName.GET_DEPENDENCIES_BY_ENTITY,
  title: 'Get dependencies by entity',
  description: 'Get entities the target explicitly depends on, using reverse dependencyOf.',
  ...
  async handler({ input, context }) {
    return catalogResult(
      queryRelatedEntities(context.catalogClient.queryEntities.bind(context.catalogClient), {
        relation: BackstageRelation.DEPENDENCY_OF,
        targetRef: input.entityRef,
      }),
      'Backstage could not complete the get dependencies by entity lookup'
    );
  },
});
