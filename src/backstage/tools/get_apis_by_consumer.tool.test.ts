...
const INPUT = { consumerRef: 'component:default/smoke-service' };

describe('get_apis_by_consumer tool', () => {
  it('should expose a valid read-only contract and send a filtered Catalog request', async () => {
    expect(getApisByConsumerTool.annotations).toMatchObject({ readOnlyHint: true, destructiveHint: false });
    expect(getApisByConsumerTool.inputSchema.safeParse(INPUT).success).toBe(true);
    expect(getApisByConsumerTool.inputSchema.safeParse({ unexpected: true }).success).toBe(false);
    const fetcher = vi
    ...
    try {
      const result = await connection.client.callTool({ name: 'get_apis_by_consumer', arguments: INPUT });
      ...
      if (request instanceof Request) {
        ...
        expect(decodeURIComponent(request.url)).toContain('relations.apiConsumedBy=component:default/smoke-service');
  ...
  it('should map Catalog rejection to a typed MCP error', async () => {
    ...
    const connection = await connectTestClient(createBackstageServer({ catalogClient, logger: noopLogger }));
    try {
      expect(await connection.client.callTool({ name: 'get_apis_by_consumer', arguments: INPUT })).toMatchObject({
        isError: true,
